...
const TaskDashboard = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8002/analytics/dashboard', {
          headers: {
  ...
  const getAiAdvice = async (taskId: string) => {
    try {
      const response = await fetch(`http://127.0.0.1:8002/tasks/${taskId}/ai-advice`, {
        headers: {
          'Authorization': `Bearer ${authToken || ''}`,
          'Content-Type': 'application/json',
        },
  ...
  const sortTasksByAI = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8002/tasks/ai-sort', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken || ''}`,
          'Content-Type': 'application/json',
